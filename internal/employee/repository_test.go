...
func Test_Store(t *testing.T) {
	...
	t.Run("Create Ok", func(t *testing.T) {
		es, err := repoProd.Create(emply12.ID, emply12.CardNumberID, emply12.FirstName, emply12.LastName, emply12.WarehouseID)
		assert.Equal(t, err, err)
		assert.Equal(t, es, es)
	})

	t.Run("Create Fail", func(t *testing.T) {
		es, err := repoProdError.Create(emply12.ID, emply12.CardNumberID, emply12.FirstName, emply12.LastName, emply12.WarehouseID)
		assert.Equal(t, err, err)
		assert.Equal(t, es, es)
	})
